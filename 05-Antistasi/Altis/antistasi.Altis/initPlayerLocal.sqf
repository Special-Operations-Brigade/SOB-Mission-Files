...
if (player getVariable ["pvp",false]) exitWith
	...
	if (side player == malos) then
		{
		if (activeUSAF or (hayIFA and worldName != "chernarus_summer")) then {[player] call A3A_fnc_RHSdress};
		}
	else
		{
		if (activeAFRF or (hayIFA and worldName != "chernarus_summer")) then {[player] call A3A_fnc_RHSdress};
		};
...
if (activeGREF or (hayIFA and worldName != "chernarus_summer")) then {[player] call A3A_fnc_RHSdress};
