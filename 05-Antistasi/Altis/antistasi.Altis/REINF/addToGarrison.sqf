...
if (spawner getVariable _cercano != 2) then
	...
	_wp setWaypointStatements ["true","[(group this),false] spawn A3A_fnc_digIn; {_x setVariable [""spawner"",nil,true]} forEach (units (group this))"];
	{
...
if (!_noBorrar) then
	...
	{_x setVariable ["spawner",true,true]} forEach (units (_grupo));
