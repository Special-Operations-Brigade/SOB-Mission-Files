...
private ["_grupo","_veh","_texto","_unidades","_tipo"];
...
if (_tipo == "mount") exitWith
	...
	_veh = objNull;
	{
	if (_x getVariable ["owner",grpNull] == _grupo) exitWith {_veh = _x};
	} forEach vehicles;
	if !(isNull _veh) then
		{
		_transporte = true;
...
if (_tipo == "berserk") exitWith
	{
	_grupo = (hcSelected player select 0);
	if (_grupo getVariable ["berserk",false]) then
		{
		hint format ["%1 exiting berserk mode",groupID _grupo];
		_grupo setVariable ["berserk",false];
		{_x enableAI "AUTOCOMBAT"} forEach (units _grupo);
		}
	else
		{
		hint format ["%1 entering berserk mode",groupID _grupo];
		_grupo setVariable ["berserk",true];
		{_x disableAI "AUTOCOMBAT"} forEach (units _grupo);
		_grupo setBehaviour "AWARE";
		_grupo setCombatMode "YELLOW";
		_grupo setVariable ["ambush",false];
		};
	};
if (_tipo == "ambush") exitWith
	{
	_grupo = (hcSelected player select 0);
	if (_grupo getVariable ["ambush",false]) then
		{
		hint format ["%1 exiting ambush mode",groupID _grupo];
		_grupo setVariable ["ambush",false];
		{_x enableAI "AUTOCOMBAT"} forEach (units _grupo);
		...
		{
		hint format ["%1 entering ambush mode",groupID _grupo];
		_grupo setVariable ["ambush",true];
		{_x disableAI "AUTOCOMBAT"} forEach (units _grupo);
...
if (_grupo getVariable ["ambush",false]) then {_texto = format ["%1Ambush Mode ON\n",_texto]};
if (_grupo getVariable ["berserk",false]) then {_texto = format ["%1Berserk Mode ON\n",_texto]};
