	...
	if(_varName in specialVarLoads) then
	{
		...
		if(_varName == 'minas') then
			...
			for "_i" from 0 to (count _varvalue) - 1 do
				...
				_index = allMinesCFG findIf {_x select 1 == _tipoMina};
				if (_index != -1) then {_tipoMina = (allMinesCFG select _index) select 0};
