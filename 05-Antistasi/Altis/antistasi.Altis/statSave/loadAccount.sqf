...
if (isServer and !_byPassServer) then
	...
	{unlockedBackpacks pushBack (_x select 0)} forEach ((jna_dataList select IDC_RSCDISPLAYARSENAL_TAB_BACKPACK) select {_x select 1 == -1}); unlockedBackpacks = [unlockedBackpacks, [], {getNumber(configFile >> "CfgVehicles" >> _x >> "maximumload")}, "DESCEND"] call BIS_fnc_sortBy;publicVariable "unlockedBackpacks";
