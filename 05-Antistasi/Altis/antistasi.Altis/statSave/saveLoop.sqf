...
_datos = [];
_dateToNumber = dateToNumber date;

{
_format = format ["caja%1",_x];
_date = server getVariable [_format,0];
if (_dateToNumber  < _date) then
	{
	_datos pushBack [_x,_date];
	};
} forEach puestos + puertos;

["boxCooldown",_datos] call fn_SaveStat;
