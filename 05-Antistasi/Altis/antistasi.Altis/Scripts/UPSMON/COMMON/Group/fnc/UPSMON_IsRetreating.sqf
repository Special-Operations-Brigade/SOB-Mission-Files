					...
					If (!(fleeing (leader _grp))) then
					{
						...
						If ((random 100) <= 10) then
						{
							...
							If (_AttackPos select 0 != 0 && _AttackPos select 1 != 0) then
								{
								if ((side (leader _grp) != buenos) and ([leader _grp] call A3A_fnc_hasRadio)) then {[[_attackPos,(side(leader _grp)),"",false],"A3A_fnc_patrolCA"] remoteExec ["A3A_fnc_scheduler",2]};
