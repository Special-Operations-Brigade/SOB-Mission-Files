...
_llevado setDir 180;
_timeOut = time + 60;
_action = _llevador addAction [format ["Release %1",name _llevado], {{detach _x} forEach (attachedObjects player)},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',_this])"];
