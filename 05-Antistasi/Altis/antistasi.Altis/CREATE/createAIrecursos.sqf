...
if (lados getVariable [_marcador,sideUnknown] == malos) then
	{
	_lado = malos;
	if ((random 10 <= (tierWar + difficultyCoef)) and (_frontera isEqualTo [])) then
		{
		_esFIA = true;
		};
	};
...
if (not(_marcador in destroyedCities)) then
	...
	if ((daytime > 8) and (daytime < 18)) then
		...
		_nul = [leader _grupo, _marcador, "SAFE", "SPAWNED","NOFOLLOW", "NOSHARE","RELAX","NOVEH2"] execVM "scripts\UPSMON.sqf";
		};
	};
...
for "_i" from 0 to (count _garrison - 1) do
	...
	_grupos pushBack _grupo;
	{[_x,_marcador] call A3A_fnc_NATOinit; _soldados pushBack _x} forEach units _grupo;
	if (_i == 0) then {_nul = [leader _grupo, _marcador, "SAFE", "RANDOMUP","SPAWNED", "NOVEH2", "NOFOLLOW"] execVM "scripts\UPSMON.sqf"} else {_nul = [leader _grupo, _marcador, "SAFE","SPAWNED", "RANDOM","NOVEH2", "NOFOLLOW","RELAX"] execVM "scripts\UPSMON.sqf"};
	};
