...
if (_tipoPatrol == "AIR") then
	...
	if (_tipoPatrol == "SEA") then
		...
		{
		_arrayDestinos = marcadores select {lados getVariable [_x,sideUnknown] == _lado};
		_arraydestinos = [_arrayDestinos,_posBase,"LAND"] call A3A_fnc_patrolDestinos;
		_distancia = 50;
		};
	};
...
while {alive _veh} do
	...
	if (_tipoPatrol == "AIR") then
		{
		_arrayDestinos = marcadores select {lados getVariable [_x,sideUnknown] == _lado};
		_arrayDestinos = [_arrayDestinos,position _veh,"AIR"] call A3A_fnc_patrolDestinos;
		}
	else
		{
		if (_tipoPatrol == "SEA") then
			...
			{
			_arrayDestinos = marcadores select {lados getVariable [_x,sideUnknown] == _lado};
			_arrayDestinos = [_arrayDestinos,position _veh,"LAND"] call A3A_fnc_patrolDestinos;
			};
		};
	};
