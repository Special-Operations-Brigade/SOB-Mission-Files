...
if (_tipo isEqualType []) then
	...
	{
	if (_tipo isEqualType objNull) then {_tipo = [typeOf _tipo]} else {_tipo = [_tipo]};
	};
_tipo = _tipo - [""];
if (_tipo isEqualTo []) exitWith {};
...
if (_lado != (lados getVariable [_marcador,sideUnknown])) exitWith {garrisonIsChanging = false};
/*
...
if (_modo == -1) then
	...
	for "_i" from 0 to (count _garrison -1) do
		{
		if (_subTipo == (_garrison select _i)) exitWith
			{
	...
	if (!_found) then
		{
		if !(_garrison isEqualTo []) then {_garrison deleteAt 0};
		};
	} forEach _tipo;
	}
else
