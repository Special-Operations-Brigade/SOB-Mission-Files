...
if ((random 100 < ((prestigeNATO) + (prestigeCSAT))) and (spawner getVariable _marcador != 2)) then
	...
	_civs pushBack _civ;
	_nul = [_civ, _marcador, "SAFE", "SPAWNED","NOFOLLOW", "NOVEH2","NOSHARE","Relax"] execVM "scripts\UPSMON.sqf";
	};
...
for "_i" from 0 to 2 do
	{
	_civ = _grupo createUnit [selectRandom arrayCivs, _pos, [],0, "NONE"];
	_nul = [_civ] spawn A3A_fnc_CIVinit;
	_civs pushBack _civ;
	};
_nul = [leader _grupo, _marcador, "SAFE", "SPAWNED","NOFOLLOW", "NOVEH2","NOSHARE","Relax"] execVM "scripts\UPSMON.sqf";
