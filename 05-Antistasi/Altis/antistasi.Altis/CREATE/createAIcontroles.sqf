...
if (_esControl) then
	...
	if (!_esFIA) then
		{
		_grupoE = grpNull;
		_check = if (_lado == malos) then {random 100 < prestigeNATO} else {random 100 < prestigeCSAT};
		if ((!hayIFA) and _check) then
			{
		...
		_tipogrupo = if (_check) then {if (_lado == malos) then {selectRandom gruposNATOmid} else {selectRandom gruposCSATmid}} else {if (_lado == malos) then {gruposNATOSentry} else {gruposCSATSentry}};
		...
		{
		_check = random 100 < prestigeNATO;
		_tipoVeh = if ((!hayIFA) and _check) then {vehFIAArmedCar} else {vehFIACar};
		_veh = _tipoVeh createVehicle getPos (_roads select 0);
		_veh setDir _dirveh + 90;
		_nul = [_veh] call A3A_fnc_AIVEHinit;
		_vehiculos pushBack _veh;
		sleep 1;
		_tipogrupo = if (_check) then {selectRandom gruposFIAMid} else {selectRandom gruposFIASmall};
		_grupo = if !(hayIFA) then {[_posicion, _lado, _tipoGrupo,false,true] call A3A_fnc_spawnGroup} else {[_posicion, _lado, _tipoGrupo] call A3A_fnc_spawnGroup};
		if !(isNull _grupo) then
			{
