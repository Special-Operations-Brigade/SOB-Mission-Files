...
for "_i" from 0 to _var1 do
	...
	if (!(_cosa in unlockedWeapons)) then
		{
		_num = 1+ (floor random 12) + tierWar;
...
for "_i" from 0 to _var2 do
	...
	if (!(_cosa in unlockedMagazines)) then {_crate addMagazineCargoGlobal [_cosa, 10 + tierWar]};
	};
for "_i" from 1 to _var3 do
	{
	_cosa = selectRandom minasAAF;
	_num = 1 + (floor random 5) + tierWar;
...
if !(hayIFA) then
	...
	for "_i" from 1 to _var4 do
		...
		if (not(_cosa in unlockedItems)) then
			{
			_crate addItemCargoGlobal [_cosa, 1 + (floor random 2) + tierWar];
			};
		};
