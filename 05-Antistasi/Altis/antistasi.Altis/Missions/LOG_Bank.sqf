...
[[buenos,civilian],"LOG",[format ["We know Gendarmes are guarding a big amount of money in the bank of %1. Take this truck and go there before %2:%3, hold the truck close to tha bank's main entrance for 2 minutes while sweep the building, and the money will be transferred to the truck. Bring it back to HQ and the money will be ours.",_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"Bank Robbery",_mrkfin],_posicion,false,0,true,"Interact",true] call BIS_fnc_taskCreate;
...
for "_i" from 1 to 4 do
	...
	sleep 1;
	if (random 2 < 1) then {_nul = [leader _grupo, _mrk, "SAFE","SPAWNED", "NOVEH2", "FORTIFY"] execVM "scripts\UPSMON.sqf"} else {[_grupo,true] spawn A3A_fnc_digIn};
...
waitUntil {sleep 1; (dateToNumber date > _fechalimnum) or (!alive _camion) or ((_camion distance _posicion < 7) and ({(side group _x == malos) and ([_x] call A3A_fnc_canFight) and (_x inArea _mrk)} count allUnits == 0))};
_bonus = if (_dificil) then {2} else {1};
if ((dateToNumber date > _fechalimnum) or (!alive _camion)) then
	{
	["LOG",[format ["We know Gendarmes are guarding a big amount of money in the bank of %1. Take this truck and go there before %2:%3, hold the truck close to tha bank's main entrance for 2 minutes while sweep the building, and the money will be transferred to the truck. Bring it back to HQ and the money will be ours.",_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"Bank Robbery",_mrkfin],_posicion,"FAILED","Interact"] call A3A_fnc_taskUpdate;
	...
	_exit = false;
	while {(_cuenta > 0) or ((_camion distance _posicion < 7) and ({(side group _x == malos) and ([_x] call A3A_fnc_canFight) and (_x inArea _mrk)} count allUnits == 0)) and (alive _camion) and (dateToNumber date < _fechalimnum)} do
		{
		while {(_cuenta > 0) and ((_camion distance _posicion < 7) and ({(side group _x == malos) and ([_x] call A3A_fnc_canFight) and (_x inArea _mrk)} count allUnits == 0)) and (alive _camion)} do
			{
