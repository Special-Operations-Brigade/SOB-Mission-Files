...
switch _tipo do
	...
	case "heal1":
		...
		if (player != _flag) then
			...
			_accion = _flag addAction [format ["<t>Carry %1</t> <img image='\A3\ui_f\data\igui\cfg\actions\take_ca.paa' size='1.6' shadow=2 />",name _flag], "Revive\carry.sqf",nil,5,true,false,"","(isPlayer _this) and (_this == _this getVariable ['owner',_this]) and (isNull attachedTo _target) and !(_this getVariable [""ayudando"",false]);",4];
