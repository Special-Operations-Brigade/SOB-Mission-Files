...
if (_medico != _unit) then
		...
		if ((_unit getVariable ["INCAPACITATED",false]) and (!isNull _enemy) and (_timeOut >= time) and (_medico != _unit)) then
			...
			if (count _cobertura == 3) then
				...
				if ([_medico] call A3A_fnc_canFight) then
					...
					//[_medico] spawn {sleep 4.5; (_this select 0) playMove "AcinPknlMwlkSrasWrflDb"};//lo pone a andar
					_timeOut = time + 30;
					while {true} do
						{
						...
						if (speed _dummy > 0) then {_medico playMove "AcinPknlMwlkSrasWrflDb"; sleep 1} else {_medico playMove "AcinPknlMstpSrasWrflDnon"; sleep 0.5};
				...
				if ((alive _unit) and ([_medico] call A3A_fnc_canFight) and (_medico == vehicle _medico) and !(_medico getVariable ["cancelRevive",false])) then
					{
					//_medico playMove "amovpknlmstpsraswrfldnon";//lo para
					_medico playMove "AcinPknlMstpSrasWrflDnon_AmovPknlMstpSrasWrflDnon";
					_medico stop true;
					_unit stop true;
					sleep 2;
			...
			/*if ((animationState _medico == "amovpknlmstpsraswrfldnon") or (animationState _medico == "AmovPercMstpSlowWrflDnon_AcinPknlMwlkSlowWrflDb_2") or (animationState _medico == "AmovPercMstpSnonWnonDnon_AcinPknlMwlkSnonWnonDb_2")) then
				{
				_medico switchMove "";
				_medico setUnconscious true;
				sleep 3;
				_medico setUnconscious false
				};*/
			}
		else
			{
			...
			if (_unit getVariable ["INCAPACITATED",false]) then
				...
				{
				if (_medico == _unit getVariable ["ayudado",objNull]) then {_medico action ["HealSoldier",_unit]; _curado = true};//creo que es aquí donde lo mejoré en Big Brother, portarlo a Antistasi
				};
			if (_curado) then
				{
				if (_medico != _unit) then {if (_isPlayer) then {_medico groupChat format ["You are ready %1",name _unit]}};
	...
	if !(_medico getVariable ["bPos",[]] isEqualTo []) then
		{
		[_medico,_medico getVariable ["bpos",getPos _medico],[]] spawn A3A_fnc_moveToBuildingPos;
		}
	else
		{
