...
_nombre = [_nombre] call BIS_fnc_baseVehicle;
_nombreAmmo = getText (configfile >> "CfgVehicles" >> _nombre >> "ammo");
if (_nombreAmmo != "") then
	{
	_simul = getText (configfile >> "CfgAmmo" >> _nombreAmmo >> "simulation");
	if ((_simul == "shotboundingmine") or (_simul == "shotmine")) then {allMinesCFG pushBackUnique [_nombre,_nombreAmmo]}
	};
} forEach _allMinesCFG;
...
_uniform = (_loadOut select 3) select 0;
...
_mochi = _loadOut select 5;
if !(_vest isEqualTo []) then {unlockedItems pushBackUnique (_vest select 0)};
if !(_mochi isEqualTo []) then {unlockedBackpacks pushbackUnique (_mochi select 0)};
} forEach [SDKSniper,SDKATman,SDKMedic,SDKMG,SDKExp,SDKGL,SDKMil,SDKSL,SDKEng,[SDKUnarmed],[staticCrewBuenos]];

if (hayRHS) then {unlockedItems = unlockedItems - ["rhs_6sh46"]};
...
unlockedBackpacks = unlockedBackpacks select {getNumber(configFile >> "CfgVehicles" >> _x >> "maximumload") > 0};
unlockedBackpacks = [unlockedBackpacks, [], {getNumber(configFile >> "CfgVehicles" >> _x >> "maximumload")}, "DESCEND"] call BIS_fnc_sortBy;
