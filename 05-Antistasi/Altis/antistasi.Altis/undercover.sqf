private ["_player"];
if (tierWar > 7) exitWith {hint "You cannot become Undercover in the current War Level"};
...
if (vehicle _player != _player) then
	...
	if (vehicle _player getVariable ["reported",false]) then
		{
...
switch _cambiar do
	...
	case "Reported":
		...
		if (vehicle _player != _player) then
			...
			vehicle _player setVariable ["reported",true,true];
			}
		else
			{
