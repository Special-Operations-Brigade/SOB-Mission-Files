[[],{
	...
	CIVPOP_fnc_startSystem = {
		...
		allCivilianClassNames = [
			"mti_factions_cishuman_unit_Officer"
		];
		...
		allIDAPClassNames = [
			"OPTRE_UEG_Civilian_Rural"
		];

		allIDAPVehicleClassNames = [
			"mti_factions_civilian_105K",
			"mti_factions_civilian_AAC",
			"mti_factions_civilian_ITT_Ambulance",
			"mti_factions_civilian_ITT",
			...
			"OPTRE_M12_FAV_PD",
			"OPTRE_M914_RV_PD",
			"mti_armoury_vehicles_tx223_base",
			"mti_armoury_vehicles_px10_r3",
			"mti_armoury_vehicles_px10_base",
			"mti_armoury_vehicles_mutt_i",
			"mti_armoury_vehicles_mutt_l",
			"mti_armoury_vehicles_isp_base",
			"mti_armoury_vehicles_bug_armed",
			"mti_armoury_vehicles_tx130_base",
			"mti_factions_mercs_TX",
			...
			"mti_factions_rebels_V25",
			"mti_factions_mercs_Jackal_MG",
			"mti_factions_mercs_GAT",
			"mti_factions_ION_RTT",
			"mti_factions_ION_LT_Cannon",
		...
		CIVPOP_fnc_initIDAPVehicles = {
			...
			[_maxUnitCount,_maxRoads,_minTravenDistance,_maxTravelDistance,_deleteDistance,_updateRate,_debug] spawn {
				...
				while {isCivilianGroundVehiclesAllowed && isPopulateWorldAllowed} do 
				{
					...
					comment "delete abandoned IDAP vehicles";
					{
						_min = 1e10;
						_veh = _x;

						_isAbandonedVehicle = _veh getVariable "isAbandonedVehicle";
						if (isNil "_isAbandonedVehicle") then {continue};

						{
							_player = _x;
							_pos = [_player,250,30] call CIVPOP_fnc_getVelocityAdjustedPosition;
							_dist = _pos distance getPosATL _veh;
							if(_dist < _min) then {_min = _dist;};
						} foreach ([includeZeus] call CIVPOP_fnc_getAllPlayers);
						if(_min > (_deleteDistance)) then 
						{
							_i = crew _veh findIf {_x in allPlayers};
							if(_i != -1) exitWith {};
							deleteVehicle _veh;
						};
					...
					comment "Spawn abandoned IDAP vehicle";
					if (count AllIDAPVehicleOwners < _modifiedUnitCount) then 
					{
						...
						_veh setFuel (0.05 + random 0.15); // Random fuel between 5-20%
						_veh setDamage (0.3 + random 0.4); // Random damage between 30-70%
						_veh addEventHandler ['Hit',{['play', _this select 0] call BIS_fnc_carAlarm;}];
						_veh setVariable ["isAbandonedVehicle",true];
						_veh setVariable ["isCivPopVehicle",true];
						AllIDAPVehicleOwners pushBack _veh;

						comment "DEBUG";
						if (_debug) then 
						{
							_mkr = createMarker["DebugMarker_Abandoned_Veh_"+str count allMapMarkers,getPos _veh];
							_mkr setMarkerType "mil_box";
							_mkr setMarkerSize [0.5,0.5];
							_mkr setMarkerColor "colorGreen";
							[_mkr,_veh] spawn {while {isCivilianGroundVehiclesAllowed && isPopulateWorldAllowed && !isNull (_this select 1)} do 
							{sleep 0.1; (_this select 0) setMarkerPos (getPos (_this select 1))}; deleteMarker (_this select 0);};
						};
					};
					sleep _updateRate;
			...
			comment "No movement update needed for abandoned vehicles - they stay where they are";
		};
		...
		_deletionDistance = 700;

		[2,_minSpawnDistance,_deletionDistance,false] spawn CIVPOP_fnc_startValidSpawnDiscovery;

		comment "Spawn Zombies (Pedestrians disabled)";
		_maxUnitCount = 0; // Disabled - no pedestrians
		_maxRoads = 1000;
		...
		comment "Spawn IDAP Personnel (Pedestrians disabled)";
		_maxUnitCount = 0; // Disabled - no IDAP pedestrians
		_maxRoads = 1000;
		...
		_maxUnitCount = 20; // Increased from 4 for more IDAP vehicle presence
		_maxRoads = 1000;
		...
		comment "Additional crewed vehicles disabled - vehicles are now abandoned";
		/*
		// Spawn additional crewed IDAP vehicles 
		[] spawn {
			private _maxCrewedVehicles = 20; // Additional crewed vehicles
			...
			while {isPopulateWorldAllowed} do {
				...
				// Delete vehicles too far from players
				{
					...
					if (_minDist > 2000) then {
						// Delete crew first if they exist
						{deleteVehicle _x} forEach crew _veh;
						deleteVehicle _veh;
					};
				} forEach _spawnedVehicles;
				// Spawn new crewed vehicles if below max
				if (count _spawnedVehicles < _maxCrewedVehicles) then {
					...
					_veh setFuel 0.12; // Slightly higher fuel for crewed vehicles
					_veh setDamage 0.3; // Less damage for active vehicles
					_veh setVariable ["isCivPopCrewedVehicle", true];
					
					// Create crew for the vehicle
					private _crewCount = 1 + floor(random 3); // 1-3 crew members
					private _allCrew = [];
					for "_i" from 1 to _crewCount do {
						private _crewClass = selectRandom allIDAPClassNames;
						private _crewMember = createAgent [_crewClass, getPos _spawn, [], 3, "CAN_COLLIDE"];
						_crewMember setVariable ["isCivPopAgent", true];
						_crewMember setVariable ["factionType", "IDAP"];
						_crewMember setVariable ["ownedVehicle", _veh];
						_crewMember disableAI "FSM";
						_crewMember allowFleeing 0;
						
						// Assign to vehicle positions
						if (_i == 1) then {
						...
						_allCrew pushBack _crewMember;
						sleep 0.1; // Small delay between crew creation
					};
					
					// Set vehicle movement behavior
					if (count _allCrew > 0) then {
						private _driver = driver _veh;
						if (!isNull _driver) then {
							[_driver, 800, 1200, true] call CIVPOP_fnc_moveToNearbyRandomRoad;
						};
					};
					
					_spawnedVehicles pushBack _veh;
				};
				sleep 15; // Slightly longer delay for crewed vehicles
			};
		};
		*/
	};
